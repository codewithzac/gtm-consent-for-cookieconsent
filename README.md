...
| Event name: ( event ), consent changes: ( n ) | The event name from whatever triggered the CookieConsent tag, commonly:<ul><li>**gtm.init_consent**: Consent Initialisation (on page load)</li><li>**onFirstConsent**: The user made their first consent choice</li><li>**onChange**: CookieConsent has detected a change of consent state from the previous state</li></ul>The number of consent changes is the number of differences between the current and previous consent states, and determines whether the `consent_update` event is fired for `onChange` events - needs to be above 0. |
